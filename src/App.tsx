...
export function App() {
  ...
  return (
    <div className="relative w-screen h-screen bg-black text-neutral-100 overflow-hidden font-sans select-none">
      <header className="absolute top-3 left-3 right-3 z-30 pointer-events-none flex flex-col gap-2.5">
        <div className="grid grid-cols-[1fr_auto_1fr] items-center w-full gap-2">
          <div className="flex items-center space-x-2 pointer-events-auto bg-black border border-neutral-800 px-3 py-1.5 rounded-lg shadow-xl shrink-0 justify-self-start">
            ...
            <div className="flex items-center space-x-1 text-[11px] font-mono text-neutral-400">
              ...
              <span className="hidden sm:inline">{allNodes.length.toLocaleString()} Systems</span>
              <span className="sm:hidden">{allNodes.length.toLocaleString()}</span>
            </div>
          </div>

          <div className="inline-flex items-center gap-1 sm:gap-1.5 p-1 bg-black rounded-lg border border-neutral-800 shadow-xl text-[11px] sm:text-xs font-mono pointer-events-auto overflow-x-auto max-w-[calc(100vw-360px)] justify-self-center">
            <button
            ...
            {(['unix', 'apple', 'bsd', 'windows', 'linux', 'independent'] as const).map((family) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center space-x-1 sm:space-x-1.5 px-2 sm:px-2.5 py-1 rounded transition-colors whitespace-nowrap capitalize ${
                    isActive
                      ? 'bg-neutral-800 text-white font-medium'
                      : 'text-neutral-400 hover:text-white'
                  }`}
      ...
      {isFilterOpen && (
        <aside className="absolute top-16 right-3 z-30 w-80 max-w-[calc(100vw-1.5rem)] animate-in fade-in duration-150">
          <FilterToolbar
            selectedFamilies={selectedFamilies}
