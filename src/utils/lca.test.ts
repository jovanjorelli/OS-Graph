...
describe('LCA & Ancestry Algorithms', () => {
  ...
  it('safely handles cyclic lineage graphs without infinite loops', () => {
    const cyclicMap = new Map<string, string[]>([
      ['nodeA', ['nodeB']],
      ['nodeB', ['nodeC']],
      ['nodeC', ['nodeA', 'nodeD']],
      ['nodeD', []],
    ...
    const paths = computeAncestryPaths('nodeA', cyclicMap);
    expect(paths.has('nodeA')).toBe(true);
    expect(paths.has('nodeB')).toBe(true);
    expect(paths.has('nodeC')).toBe(true);
    expect(paths.has('nodeD')).toBe(true);
  });
});
